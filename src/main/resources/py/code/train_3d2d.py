...
if snapshot_path is None or not os.path.exists(snapshot_path):
    print("缺少 snapshot_path 或路径无效，请从Java或命令行正确传参")
    sys.exit(1)

print(f"从Java接收到snapshot_path: {snapshot_path}")
...
if __name__ == "__main__":
    ...
    trainloader1 = DataLoader(db_train1, batch_size=batch_size, shuffle=True,  num_workers=8, pin_memory=False,  worker_init_fn=worker_init_fn)
    model1.train()
    model2.train()
    model3.train()
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch1 in enumerate(trainloader1):
           ...
           #  outputs_soft1 = F.softmax(outputs1, dim=1)   #torch.Size([1, 2, 128, 128, 128])
            ...
            outputs3=model3(volume_batch3.cpu()).cuda() #[128,8,128,128]
